...
final _crmSummaryProvider = FutureProvider<Map<String, int>>((ref) async {
  final api = ref.watch(apiClientProvider);
  try {
    final results = await Future.wait([
      api.getList('/api/v1/crm/leads/'),
      api.getList('/api/v1/crm/contacts/'),
      api.getList('/api/v1/crm/companies/'),
      api.getList('/api/v1/crm/opportunities/'),
    ]);
    return {
      'leads': results[0].length,
      'contacts': results[1].length,
      'companies': results[2].length,
      'opportunities': results[3].length,
    };
  } catch (_) {
    return {'leads': 0, 'contacts': 0, 'companies': 0, 'opportunities': 0};
  }
});

// ── Screen ─────────────────────────────────────────────────────────────────────

class CrmWorkspaceScreen extends ConsumerStatefulWidget {
  const CrmWorkspaceScreen({super.key});

  @override
  ConsumerState<CrmWorkspaceScreen> createState() => _CrmWorkspaceScreenState();
}

class _CrmWorkspaceScreenState extends ConsumerState<CrmWorkspaceScreen> {
  String _selectedCategory = 'leads';
  String _search = '';
        ...
        SliverToBoxAdapter(
          child: Padding(
            ...
            child: _ManagementCard(
              title: 'CRM Workspace',
              subtitle:
                  'Manage leads, contacts, companies, opportunities, and activities without disrupting document workflows.',
              onRefresh: () {
                ref.invalidate(_crmSummaryProvider);
        ...
        SliverToBoxAdapter(
          child: Padding(
            ...
            child: _QuickSearchField(
              ...
              placeholder: 'Search name, title, email, phone, status...',
  ...
  Future<void> _openForm(
    ...
    final saved = await showModalBottomSheet<bool>(
      ...
      useSafeArea: true,
      builder: (_) => _CrmForm(config: _currentConfig, row: row),
    );
    if (saved == true) {
      ref.invalidate(resourceListProvider(_currentConfig));
      ref.invalidate(_crmSummaryProvider);
...
class _CrmStatsGrid extends StatelessWidget {
  const _CrmStatsGrid({required this.counts});
...
class _CrmRecordCard extends ConsumerWidget {
  const _CrmRecordCard({
...
class _CrmDrawer extends ConsumerWidget {
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    const items = [
...
// ── CRM Form (generic fields) ─────────────────────────────────────────────────

class _CrmForm extends ConsumerStatefulWidget {
  const _CrmForm({required this.config, this.row});
  final dynamic config;
  final Map<String, dynamic>? row;

  @override
  ConsumerState<_CrmForm> createState() => _CrmFormState();
}

class _CrmFormState extends ConsumerState<_CrmForm> {
  ...
  Future<void> _save() async {
    ...
    final payload = {for (final e in _ctrl.entries) e.key: e.value.text.trim()};
    try {
